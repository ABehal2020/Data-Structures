...
//  lab10
//
//  Created by Aditya Behal on 4/7/21.
...
public:
    ...
    public:
        ...
        iterator(TreeNode* p, const ds_set<T>* s) : ptr_(p), set_(s) {}
        ...
        iterator & operator++() {
            ...
            cout << "Up count: " << up_count << " ";
            cout << "Down count: " << down_count << endl;
            return *this;
        }
        ...
        iterator & operator--() {
            if (ptr_ == NULL) { // so that it works for end()
                assert (set_ != NULL);
                ptr_ = set_->root_;
                while (ptr_->right != NULL) { ptr_ = ptr_->right; down_count++; }
                ...
                while (ptr_->parent != NULL && ptr_->parent->left == ptr_) { ptr_ = ptr_->parent; up_count++; }
                ptr_ = ptr_->parent;
                up_count++;
            }
            cout << "Up count: " << up_count << " ";
