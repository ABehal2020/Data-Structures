...
//  lab9
//
//  Created by Aditya Behal on 3/31/21.
...
public:
    ...
    const T& traverse(TreeNode<T>* p, T& key_value) const {
        // in order traversal (left, myself, right)
        if (p != nullptr) {
            traverse(p->left, key_value);
            key_value += p->value;
            traverse(p->right, key_value);
        }
        // pre order traversal (myself, left, right)
        /*
        if (p != nullptr) {
            key_value += p->value;
            traverse(p->left, key_value);
        ...
        */
        // post order traversal (left, right, myself)
        /*
        if (p != nullptr) {
            traverse(p->left, key_value);
            traverse(p->right, key_value);
    ...
    // O(n) as traverse is O(n) where n is the number of nodes - you need to visit all the nodes
    const T& accumulate(T& key_value) const {
        TreeNode<T>* p = root_;
        return traverse(p, key_value);
    }
...
private:
    ...
    /*
    // O(h) (equivalent to O(log n) if exactly balanced BST) where h is number of levels in the tree and n is the number of nodes in the tree
    // recursive version of find method
    ...
    */
    ...
    // O(h) (equivalent to O(log n) if exactly balanced BST) where h is number of levels in the tree and n is the number of nodes in the tree
    // iterative version of find method
    iterator find(const T& key_value, TreeNode<T>* p) {
        while (p != nullptr) {
            if (p->value > key_value) {
                p = p->left;
